...
layout(location = 4) flat in uint currentIndex;
...
layout(set = 1, binding = 3) readonly buffer InfoTextureObject {
    uint info_texture[3];
} ito;
...
void main() {
    ...
    if (ito.info_texture[currentIndex * 8] == 1) {
        texture_color = texture(color_sampler, UV);
    }
